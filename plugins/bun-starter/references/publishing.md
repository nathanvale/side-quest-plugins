...
2. PR opened:
   a. Changeset Bot comments with changeset status (instant feedback)
   b. autogenerate-changeset.yml creates one if missing (commits to PR branch)
3. PR merged to main
4. publish.yml runs Changesets action:
   a. If pending changesets exist: opens "Version Packages" PR
   b. If version PR merged: publishes to npm + creates GitHub release
```

### Changeset Bot (GitHub App)

The [Changeset Bot](https://github.com/apps/changeset-bot) is a GitHub App that comments on every PR with changeset status. It provides instant feedback before the auto-generate workflow runs.

**Install:** https://github.com/apps/changeset-bot — select your repo, grant pull request read/write and contents read-only.

**What it does:**
- PR has no changeset → comments "No Changeset found" with a link to the changesets docs
- PR has a changeset → comments confirming which packages and bump types are included

**Relationship to `autogenerate-changeset.yml`:**
- The bot is **read-only** — it only comments, never creates files
- The workflow is **write** — it generates the `.changeset/*.md` file and commits it
- Both are complementary: bot gives instant visibility, workflow handles automation
