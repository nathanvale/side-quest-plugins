...
- `--upgrade` -> set **UPGRADE_MODE = true**. Skip Steps 1-4 and go directly to Step 5: Upgrade Flow.
...
Check whether `cache/staged-intel.md` exists.
...
Read [verified-intel.md](references/verified-intel.md) if it exists. This is curated community intelligence that has been reviewed and accepted. Treat it as trusted reference material.

If the file does not exist or is empty (only the header), proceed without it.
...
3. Check verified-intel.md for previously confirmed similar issues
...
### Example 4: Upgrade flow

```text
User: /hooks --upgrade

Skill: [Detects --upgrade flag, skips Steps 1-4, goes to Step 5]
Skill: [Extracts unreviewed findings from staged data]
Skill: "Found 3 new community findings..."
User: "1,3" (picks findings to accept)
Skill: [Appends accepted to verified-intel.md, records all decisions]
Skill: "Added 2 findings to verified-intel.md. Rejected 1."
```

## Step 5: Upgrade Flow (--upgrade mode)

This step only runs when UPGRADE_MODE is true. Skip Steps 1-4 entirely.
...
Options: "All", "None", "Let me pick"

If user selects "Let me pick", ask them to specify numbers (e.g., "1,3").

### 5c. Process decisions

If "All": accept all findings.
If "None": reject all findings.
If specific numbers: accept those, reject the rest.

**For accepted findings**, append each to `references/verified-intel.md` using the Write tool, in this format:
...
For accepted hashes:
```bash
bunx @side-quest/community-intel-cache review --cache-dir "${CLAUDE_PLUGIN_ROOT}/skills/hooks/cache" --hashes hash1,hash2,... --decision accepted
```

For rejected hashes:
```bash
bunx @side-quest/community-intel-cache review --cache-dir "${CLAUDE_PLUGIN_ROOT}/skills/hooks/cache" --hashes hash1,hash2,... --decision rejected
```
