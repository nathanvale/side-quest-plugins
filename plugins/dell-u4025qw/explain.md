# How the Dell U4025QW Community Intelligence System Works
...
1. **30-day window is lossy** - Each refresh overwrites the previous one. Important findings (firmware bugs, confirmed workarounds) vanish when they age out of the search window.
...
1. **Background hook** - A SessionStart hook silently refreshes the cache every 30 days using `@side-quest/community-intel-cache`. You never see it, you never wait for it.
...
3. **Upgrade flow** - Run `/tech-support --upgrade` to review staged findings, accept or reject them. Accepted findings are appended to verified-intel.md and persist forever.
...
Defines the 6 research topics, refresh intervals, and context for LLM synthesis. Shared by the CLI package.
...
```bash
bunx @side-quest/community-intel-cache reset --cache-dir ./skills/dell-u4025qw/cache
```
...
```
Day 0:   First session -> no cache -> async refresh -> staged files written (30d interval)
Day 1:   Session start -> cache fresh -> exit <1ms
Day 1:   /tech-support "firmware?" -> answers from reference files + verified-intel.md
         Footer: "3 new community findings available. Run /tech-support --upgrade to review."
Day 1:   /tech-support --upgrade -> reviews findings -> 2 accepted to verified-intel.md
Day 15:  Session start -> cache fresh -> exit <1ms
Day 30:  Session start -> cache stale -> async refresh -> new staged files
Day 30:  /tech-support "disconnect" before hook finishes
         -> stale + troubleshooting -> smart refresh (~60s) -> answer
Day 31:  /tech-support --upgrade -> reviews new batch of findings
...
```
