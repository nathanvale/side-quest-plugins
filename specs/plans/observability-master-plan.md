...
**1. Correlation ID stability in OBS-8:**
Keep the existing single `correlationId` model in `EventEnvelope` for this phase. Generation remains server-side during enrichment via `generateCorrelationId()`. Any migration to a multi-tier correlation hierarchy is explicitly out of scope for OBS-8 and requires a separate compatibility plan.
...
Server enrichment pipeline uses `{ event, correlationId, sessionId, durationMs, officer, ... }` properties pattern (not template literals) for jq queryability. Hooks have no logging (dumb pipe -- zero overhead).
...
**6. EventEnvelope contract for OBS-8:**
```typescript
interface EventEnvelope<T = unknown> {
  ...
  readonly correlationId: string   // generated by server during enrichment
  readonly data: T
}
```

OBS-8 keeps this contract unchanged to avoid breaking existing server/client readers and tests.
...
**2. Forward-compatible with OTel GenAI Semantic Conventions (without OBS-8 schema changes):**
- Keep the current envelope contract in OBS-8; avoid a migration while full hook coverage is landing.
- If OTel export becomes a requirement, add an explicit mapping/migration design in a separate plan with consumer compatibility guarantees.
...
OBS-8 keeps `correlationId` to preserve server/client compatibility. Any envelope migration is a separate post-OBS-8 track.
