...
function generateTopicSection(result: ResearchResult): string {
	...
	if (result.reddit?.threads?.length) {
        ...
        <div class="source">
          ...
          <div class="source-meta">
            <span class="engagement">↑ ${thread.upvotes}</span>
            <span class="engagement">💬 ${thread.comments}</span>
          </div>
	...
	if (result.x?.posts?.length) {
        ...
        <div class="source">
          ...
          <div class="source-meta">
            <span class="engagement">❤️ ${post.likes}</span>
            <span class="engagement">🔄 ${post.reposts}</span>
...
function generateTopicTextSection(result: ResearchResult): string {
	...
	if (result.reddit?.threads?.length) {
		...
		for (const thread of result.reddit.threads.slice(0, 3)) {
			lines.push(`  • r/${thread.subreddit}: ${thread.title}`)
			lines.push(`    ↑${thread.upvotes} 💬${thread.comments} - ${thread.url}`)
		}
		lines.push('')
	}
	...
	if (result.x?.posts?.length) {
		...
		for (const post of result.x.posts.slice(0, 3)) {
			lines.push(`  • @${post.handle}: ${truncate(post.content, 100)}`)
			lines.push(`    ❤️${post.likes} 🔄${post.reposts} - ${post.url}`)
		}
		lines.push('')
	}
